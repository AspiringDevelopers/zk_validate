...
const abi = parseAbi([
  'function submitProof(bytes calldata _proof, uint256[] calldata _publicSignals) external'
]);
...
export default function Home() {
  ...
  const handleGenerateProofSendTransaction = async (e: any) => {
    ...
    try {
      ...
      // Write the transaction using wagmi v2
      writeContract({
  ...
  const renderSubmitButton = () => {
    return (
      <ClientOnly fallback={<Button disabled>Loading...</Button>}>
        {!isConnected ? (
          <ConnectWalletButton />
        ) : (
          <Button 
          ...
          >
            {isPending ? "Confirming..." : isConfirming ? "Processing..." : "Generate Proof & Send Transaction"}
          </Button>
        )}
      </ClientOnly>
    )
